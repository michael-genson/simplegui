...
if __name__=='__main__':
    ...
    import simpleicons
    import random
    test_img = 'E:\\OneDrive\\Documents\\Python Scripts\\sample files\\peppers.png'

    test_app = simpleapp.App('test app', start_hidden=True)

    view=simpleviews.SimpleView(test_app)
    nestedview=simpleviews.SimpleView(view, scrollable=True, hierarchy='secondary')

    test_checkbutton = tk.BooleanVar()
    test_radiobutton = tk.StringVar()
    menu = SimpleMenu(view, {
        'File': {
            'New': {
    ...
    widget=SimpleEntry(view)
    context_menu = SimpleMenu(widget, {
            'Print success!': lambda: print('success!')
        }, context_menu=True)

    widget1=SimpleTextbox(view)
    widget2=SimpleButton(view, 'enable', lambda: menu.enable({'Edit': {'radiobutton test': 'label1'}}))
    widget3=SimpleButton(view, 'disable', lambda: menu.disable({'Edit': {'radiobutton test': 'label1'}}))
    widget4=SimpleButton(view, 'test1', lambda: print(test_checkbutton.get()))
    widget5=SimpleButton(view, 'test2', lambda: print(test_radiobutton.get()))

    seperator = SimpleSeparator(view, orient='horizontal')

    nestedwidget1=SimpleLabel(nestedview, 'nested1')
    nestedwidget2=SimpleLabel(nestedview, 'nested2')
    nestedwidget3=SimpleLabel(nestedview, 'nested3')
    nestedwidget4=SimpleLabel(nestedview, 'nested4')
    nestedwidget5=SimpleLabel(nestedview, 'nested5')
    nestedwidget6=SimpleLabel(nestedview, 'nested6')

    nestedview.build_grid({
        'row1': nestedwidget1,
        'row2': nestedwidget2,
        'row3': nestedwidget3,
        'row4': nestedwidget4,
    ...
    view.build_grid({
        'row1': [widget, widget2, widget3, widget4, widget5],
        'row2': [seperator, 'ext', 'ext', 'ext', 'ext'],
        'row3': [nestedview]
    })
    view.pack()
