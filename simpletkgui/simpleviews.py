...
if __name__ == '__main__':
    ...
    appname = 'testapp'
    test_app = App(appname, start_hidden=True)
    test_window = SimpleWindow(test_app, windowname='test child window', start_hidden=True)

    main_view = SimpleView(test_app, Nav(test_app, title='Gui Test App'))
    main_view.build_grid({
            'row1': [SimpleButton(main_view, 'Simple View Test', lambda: test_app.change_view(test_simple_view_1))],
            'row2': [SimpleButton(main_view, 'test_gm', lambda: test_app.change_view(test_gm))],
            'row3': [SimpleButton(main_view, 'child window', test_window.show)]
        })

    test_nav_1 = Nav(test_app, return_view_text='Main Menu', return_view=main_view, title='Test View', page_list=[0,1,2], page_func=lambda list_value: print(f'New page! {list_value}'), button_1_text='Test 1', button_1_func=lambda: test_app.change_view(test_simple_view_1), button_2_text='Test 2', button_2_func=lambda: test_app.change_view(test_simple_view_2))
    test_simple_view_1 = SimpleView(test_app, test_nav_1)
    test_simple_view_2 = SimpleView(test_app, test_nav_1)
    
    test_simple_view_1.build_grid({
        'row1': [
            SimpleLabel(test_simple_view_1, 'Simple Label Test 1'),
            SimpleLabel(test_simple_view_1, 'Simple Label Test 2')
            ],

        'row2': [
            SimpleLabel(test_simple_view_1, 'Simple Label Test 3', font=test_simple_view_2.style.font.h3),
            'ext'
            ],
        ...
        'row6': None,
        'row9': [
            None, SimpleLabel(test_simple_view_1, 'Simple Label Test 4', font=test_simple_view_2.style.font.body)
            ]
        }, even_rows=False)

    test_simple_view_2.build_grid({
        'row1': [
            SimpleLabel(test_simple_view_2, text='new view test!', font=test_simple_view_2.style.font.h3)
            ]
        })
    test_gm = GridView(test_app, test_nav_1)
    test_gm.add_widgets([
            ((0,0),(1,1), SimpleLabel(test_gm, 'test_gm')),
            ((4,0),(1,4), SimpleSeparator(test_gm, 'vertical')),
            ((1,1),(1,1), SimpleLabel(test_gm, 'test_gm')),
            ((2,2),(1,1), SimpleLabel(test_gm, 'test_gm')),
            ((5,3),(1,1), SimpleLabel(test_gm, 'test_gm')),
        ])

    test_gm.build_grid(even_rows=False)

    test_app.change_view(main_view)
    print(test_app.appconfig['test'])
    test_app.appconfig.save()
